    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <section className="mb-8">
                ...
                {finished.length === 0 && <div className="text-coffee-bean-600">Aucune histoire terminée.</div>}
                        ...
                        <li key={a.story.id} className="p-4 border rounded-md shadow-sm bg-white">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="text-sm text-coffee-bean-600">Fins atteintes : {a.endingsReached ?? 0}</div>
                                </div>
                                <div className="text-right">
                                    <Link to={`/story/${a.story.id}`} className="text-cherry-rose-500 hover:underline">Voir l'histoire</Link>
                                </div>
                            </div>
                        </li>
                    ))}
            ...
            <section>
                ...
                {inProgress.length === 0 && <div className="text-coffee-bean-600">Aucune lecture en cours.</div>}
                <ul className="space-y-3 mt-3">
                    {inProgress.map(a => (
                        <li key={a.story.id} className="p-4 border rounded-md shadow-sm bg-white flex justify-between items-center">
                            <div>
                                ...
                                <div className="text-sm text-coffee-bean-600">Progression : {typeof a.progress === 'number' ? `${a.progress}%` : '—'}</div>
                                <div className="text-sm text-coffee-bean-600">Fins atteintes : {a.endingsReached ?? 0}</div>
                            </div>
                            <div>
                                <Link
                                    to={a.lastSessionId ? `/read/${a.lastSessionId}` : `/story/${a.story.id}`}
                                    className="px-3 py-2 bg-cherry-rose-500 text-white rounded hover:bg-cherry-rose-600"
                                >
                                    Continuer
                                </Link>
                            </div>
